...
class Menu extends \yii\widgets\Menu
{
     ...
     * @inheritdoc
     */
    public $linkTemplate = '<a href="{url}" class="nav-link">{icon}<p>{label}</p></a>';
    /**
     * @inheritdoc
    ...
    public $defaultIconHtml = '<i class="nav-icon fas fa-angle-right"></i> ';
    public $options = ['class' => 'nav nav-pills nav-sidebar flex-column nav-flat', 'data-widget' => 'treeview', 'role'=>'menu', 'data-accordion'=>'true'];
    ...
    {
        if (isset($item['items'])) {
            $labelTemplate = '<a href="{url}" class="nav-link">{icon} <p>{label} <i class="right fas fa-angle-left"></i></p></a>';
            $linkTemplate = '<a href="{url}" class="nav-link">{icon} <p>{label} <i class="right fas fa-angle-left"></i></p></a>';
        } else {
        ...
        if ($item['active']) {
            $labelTemplate = str_replace("nav-link", 'nav-link active', $labelTemplate);
            $linkTemplate = str_replace("nav-link", 'nav-link active', $linkTemplate);
        }

        $replacements = [
            '{label}' => strtr($this->labelTemplate, ['{label}' => $item['label'],]),
            '{icon}' => empty($item['icon']) ? $this->defaultIconHtml
                : '<i class="nav-icon ' . static::$iconClassPrefix . $item['icon'] . '"></i> ',
    ...
    protected function renderItems($items)
    {
        ...
        foreach ($items as $i => $item) {
            ...
            $class = [];
            if ($item['active'] && isset($item['items'])) {
                $class[] = 'menu-open';
            }
